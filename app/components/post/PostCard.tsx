      ...
      <div className="w-full max-w-sm rounded-[32px] bg-white p-6 text-center text-[#6b2f13] shadow-[0_24px_60px_rgba(107,47,19,0.22)]">
        ...
        <div className="mt-6 flex gap-3">
          <button
            ...
            onClick={onCancel}
            disabled={isDeleting}
            className="flex-1 rounded-full bg-[#fff4d7] py-3 text-sm font-black text-[#6b2f13] disabled:opacity-50"
          >
            キャンセル
          </button>
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 rounded-full bg-red-500 py-3 text-sm font-black text-white disabled:opacity-50"
          >
            削除
          </button>
        </div>
      ...
      <article className="overflow-hidden rounded-[28px] border border-white/75 bg-white/95 shadow-[0_16px_44px_rgba(107,47,19,0.13)]">
        ...
        {portalRoot && deleteCommentId !== null &&
          createPortal(
            <DeleteConfirmPopup
              title="コメントを削除しますか？"
              isDeleting={isDeletingCommentId !== null}
